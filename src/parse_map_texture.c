...
int	read_texture(t_all *a, e_map_elem elem_type)
{
	...
	if ((elem_type == TEX_NO && get_texture(a->mlx, &a->game.map.tex_no,
					a->file.split_line[1]))
		|| (elem_type == TEX_SO && get_texture(a->mlx, &a->game.map.tex_so,
				a->file.split_line[1]))
		|| (elem_type == TEX_WE && get_texture(a->mlx, &a->game.map.tex_we,
				a->file.split_line[1]))
		|| (elem_type == TEX_EA && get_texture(a->mlx, &a->game.map.tex_ea,
				a->file.split_line[1])))
